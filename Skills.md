...
portfolio_metrics:
  ...
  - selected_assets: [{ticker, name, sector, asset_type, weight}]
    # 실제 종목명·섹터 포함 → 할루시네이션 방지 핵심
  - sector_weights: {섹터명: 비중%}   # 실제 섹터별 비중 (내림차순)
  - top_holding_weight: 최대 단일 종목 비중 (%)   # 집중도 지표
  - returns: 테스트 기간 누적 수익률 (%)
  - volatility: 연환산 변동성 (%)
  - sharpe_ratio
  - alpha (%)
  - max_drawdown (%)
...
# 원칙: 섹터·종목 정보를 반드시 실제 데이터로 전달 → GPT가 없는 정보를 지어내는 것 방지
