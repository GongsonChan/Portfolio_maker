...
TUTORIAL_STEPS = [
    ...
    ("파라미터 설정",
     "핵심 파라미터를 설정하세요.\n\n"
     "**선택 종목 수**: 많을수록 분산투자(안정적), 적을수록 집중투자(고수익 가능)\n\n"
     "**스코어링 기간(M개월)**: 종목을 평가하는 기준 기간. "
     "이 기간 데이터로 종목을 선별하며, 하단 '현재 포트폴리오 추천'에도 동일 기간이 적용됩니다.\n\n"
     "**테스트 기간(최근 N개월)**: 선별된 종목으로 백테스트를 수행하는 기간. "
     "스코어링 기간 직후 N개월을 평가해 전략의 실제 성과를 확인합니다.\n\n"
     "**Core 지표 중요도 (0~10)**: 종목 선택 시 어떤 지표를 더 중시할지 비중을 설정합니다. "
     "수익률·리스크·샤프비율·알파·최대낙폭 중 높게 설정한 지표가 좋은 종목을 우선 선택합니다. "
     "슬라이더 아래 실시간 비율(%)을 확인하며 조정하세요."),
    ("분석 실행",
